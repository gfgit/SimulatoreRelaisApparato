; Setup SimulatoreRelaisApparato and register variables for file associations
...
!insertmacro MUI_PAGE_LICENSE "${SIMULATORE_RELAIS_LICENSE}"
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\${SIMULATORE_RELAIS_EXE}"
!define MUI_FINISHPAGE_NOREBOOTSUPPORT

!define MUI_FINISHPAGE_SHOWREADME "${SIMULATORE_RELAIS_README}"
...
LangString DESC_SM_Shortcut ${LANG_ENGLISH} "Create shortcuts to Start Menu. This makes easier to start ${APP_NAME}"
...
LangString DESC_FileAss ${LANG_ENGLISH} "Setup file associations to display an icon for Simulatore Relais circuits files and be able to open the application by just double clicking on the file"
LangString DESC_FileAss ${LANG_ITALIAN} "Configura le associazioni dei file per mostrare un'icona sui file Circuiti Simulatore Relais e per aprire l'applicazione semplicemente facendo doppio click sul file"
...
Section "Application" main_program
	...
	SetOutPath $INSTDIR\imageformats
        File ${SIMULATORE_RELAIS_PATH}\imageformats\*.dll
  
        ; TODO
        ;SetOutPath $INSTDIR\icons
        ;File ${SIMULATORE_RELAIS_EXTRA}\icons\lightning\lightning.svg
  
	SetOutPath $INSTDIR\translations
        File ${SIMULATORE_RELAIS_PATH}\translations\*.qm
  
        SetOutPath "$LOCALAPPDATA\${COMPANY_NAME}\${APP_PRODUCT}"
	; Create empty settings file
        FileOpen $0 $OUTDIR\${SIMULATORE_RELAIS_SETTINGS} w
	FileClose $0
	...
	${Switch} $LANGUAGE
		...
		${Default}
			${Break}
	${EndSwitch}
        WriteINIStr $OUTDIR\${SIMULATORE_RELAIS_SETTINGS} "General" "language" $0
	...
	# Registry information for add/remove programs
        ...
        WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\${COMPANY_NAME} ${APP_PRODUCT}" "DisplayIcon" "$\"$INSTDIR\relais.ico$\""
...
Section "Start Menu Shortcuts" sm_shorcuts
	...
	SetShellVarContext current
        ...
        CreateShortCut "$SMPROGRAMS\${COMPANY_NAME}\${APP_NAME}.lnk" "$INSTDIR\${SIMULATORE_RELAIS_EXE}" "" "$INSTDIR\relais.ico"
SectionEnd
...
Section "File associations" file_ass
	...
	SetOutPath $INSTDIR
        ...
        WriteRegStr HKCU "Software\Classes\.simrelaisc" "" "SIMULATORE_RELAIS.session"
        WriteRegStr HKCU "Software\Classes\.simrelaisc" "PerceivedType" "document"
        WriteRegStr HKCU "Software\Classes\SIMULATORE_RELAIS.session" "" "Simulatore Relais circuits"
        WriteRegStr HKCU "Software\Classes\SIMULATORE_RELAIS.session\DefaultIcon" "" "$INSTDIR\${SIMULATORE_RELAIS_EXE},0"
        WriteRegStr HKCU "Software\Classes\SIMULATORE_RELAIS.session\shell\open\command" "" '$INSTDIR\${SIMULATORE_RELAIS_EXE} "%1"'
...
Section un.main_program
	...
	# Remove files
        Delete $INSTDIR\${SIMULATORE_RELAIS_EXE}
        Delete $INSTDIR\relais.ico
...
delete_settings:	
        Delete "$LOCALAPPDATA\${COMPANY_NAME}\${APP_PRODUCT}\${SIMULATORE_RELAIS_SETTINGS}"
	...
	!macroend
        ...
        !insertmacro AssocDeleteFileExtAndProgId HKCU ".simrelaisc" "SIMULATORE_RELAIS.session"
